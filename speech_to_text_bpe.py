...
from ipl_loops_fix import IPL_FitLoop, IPL_TrainingBatchLoop, IPL_TrainingEpochLoop
# from ipl_epoch_loop import IPL_TrainingEpochLoop
# from ipl_batch_loop import IPL_TrainingBatchLoop
# from ipl_fit_loop import IPL_FitLoop
from ipl_ctc_bpe_model import IPLEncDecCTCModelBPE
from pytorch_lightning.callbacks import RichProgressBar

import torch


@hydra_runner(config_path="", config_name="config_ctc")
def main(cfg):
    logging.info(f'Hydra config: {OmegaConf.to_yaml(cfg)}')

    trainer = pl.Trainer(**cfg.trainer)
    
    fit_loop = IPL_FitLoop()
    training_epoch_loop = IPL_TrainingEpochLoop()
    training_batch_loop = IPL_TrainingBatchLoop()
    ...
    asr_model = IPLEncDecCTCModelBPE(cfg=cfg.model, trainer=trainer)

    # Initialize the weights of the model from another model, if provided via config
    asr_model.maybe_init_from_pretrained_checkpoint(cfg)
    
#     ckpt_path = 'conformer_ctc_ipl--val_wer=0.2469-epoch=8.ckpt'
    ckpt_path = 'conformer_ctc_ipl_libri--val_wer=0.3322-epoch=39-last.ckpt'
#     ckpt_path = 'conf_ctc26.ckpt'
    ckpt = torch.load(ckpt_path)
    asr_model.load_state_dict(ckpt['state_dict'])
...
"""
CUDA_VISIBLE_DEVICES=1 python3 speech_to_text_bpe.py
"""
